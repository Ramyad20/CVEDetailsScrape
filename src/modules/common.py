...
def load_global_config() -> dict:
	...
	dynamic_config = load_config_file('dynamic_config_template.json')
...
def calculate_cvss_rating(values: str) -> list:
    """ Given a string with the float values, returns a list with the correspond value in terms of strings.
    """
    cvss_ratings = list()
    ...
    for num in values:
        try:
            ...
            if 9.0 <= cvss <= 10.0:
                ...
                cvss_ratings.append('High')
            elif 4.0 <= cvss < 7.0:
                cvss_ratings.append('Medium')
            elif 0.1 <= cvss < 4.0:
                cvss_ratings.append('Low')
            elif cvss == 0.0:
...
def string_values_to_list_values(values: str) -> list:
    ...
    for num in values:
        if len(num) == 0:
            ...
            lista_values.append(int(num))
    return lista_values

def string_values_to_list_strings(values: str) -> list:
    """Given a string with values, returns a list with the correspond value in string type.
    """
    lista_values = list()
    ...
    for value in values:
        if len(value) == 0:
            ...
            lista_values.append(value.strip(' '))
    return lista_values
