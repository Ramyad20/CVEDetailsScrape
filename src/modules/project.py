...
class Project:
	...
	def get_project_list_from_config(config: dict = GLOBAL_CONFIG) -> list:
		...
		output_directory_diff_path = config['output_directory_diff_path']
		...
		for full_name, info in project_config.items():
			...
			info['output_directory_diff_path'] = output_directory_diff_path
			project: Project
	...
	def get_base_output_csv_path(self, prefix: str) -> str:
		...
		if prefix == 'cve':
			filename = filename[:-4] + "__.csv"
		return os.path.join(self.output_directory_path, filename)

	def find_last_diff_cves(self, subdirectory: str, project: str, input_file: str, output_file: str, desaparecidas: bool = None):
		""" Find the path to the last CVEs collected in diff automatization and
  		return the csv from new and updated cves."""
		
		# Listamos todos os ficheiros dentro da subpasta
		csv_path = subdirectory
		all_files = os.listdir(csv_path)
		latest_subdir = None
		for i in all_files:
			if project.short_name not in i:
				continue
			caminho = os.path.join(csv_path, i)
			if latest_subdir == None:
				latest_subdir = caminho
			elif os.stat(caminho).st_mtime > os.stat(latest_subdir).st_mtime:
				latest_subdir = caminho
		...
		if input_file == None:
			...
			for d in help_:
				...
				if "desaparecidas" in d and desaparecidas is not None and desaparecidas:
					all_files_needed.append(file_found)
     
			input_file = all_files_needed
			output_file = os.path.join(latest_subdir, output_file)

		# Caso contrario verificamos se o input existe e retornamos os caminhos corretos
		else:
			input_file = os.path.join(latest_subdir, input_file)
			output_file = os.path.join(latest_subdir, output_file)
	...
	def scrape_vulnerabilities_from_cve_details(self) -> Iterator[Cve]:
		...
		for vendor_id, product_id in zip(self.vendor_id, self.product_id):
			...
			for i, page_url in enumerate(page_url_list):
				...
				vulnerability_table = page_soup.find('div', id='searchresults')
				
				cve_a_list = vulnerability_table.find_all('div', class_="border-top py-3 px-2 hover-bg-light") #title=ScrapingRegex.CVE)
				...
				for j, cve_a in enumerate(cve_a_list):
					...
					cve_id = cve_a.find('a').get_text(strip=True)
					cve = Cve(cve_id, self)
